...
fun CameraScreenSuccessBroken(
    ...
    val adapter by remember {
        mutableStateOf(
            CompositeAdapter(
                mapOf(
                    SelectorItem.TextItem::class.java to TextSelectorItemUI(),
                    SelectorItem.IconItem::class.java to IconSelectorItemUI()
                )
            )
        )
    }
    ...
    val item by remember {
        mutableStateOf(VectorRadioItem(Item.WB, "WB", wb.value))
    }
  //  val radioGroupItems = remember { emptyList<RadioButton>() }

    val radioGroupItems by remember {
        mutableStateOf(
            listOf(
               // item
    ...
    LaunchedEffect(selectorSelectedItemIndex) {
        ...
        settingsRequest = when (radioGroupSelectedItem) {
            Item.SHUTTER -> {
                if (autoItems[radioGroupSelectedItem] == false) {
                    settingsRequest.copy(
                        shutterValue = characteristics.items.shutterItems?.get(
            ...
            Item.ISO -> {
                if (autoItems[radioGroupSelectedItem] == false) {
                    settingsRequest.copy(
                        isoValue = characteristics.items.isoItems?.get(
                            selectorSelectedItemIndex
                        )?.text ?: run { "" })
                } else {
