...
@Deprecated(
    ...
    replaceWith = ReplaceWith("CameraScreenSuccess()"),
    level = DeprecationLevel.ERROR
    )
@Composable
fun CameraScreenSuccessOld(
    context: Context,
    renderer: GLRenderer,
    // characteristicsInitial: CharacteristicsUI,
    characteristicsInit: CharacteristicsUI,
    ...
    var radioGroupSelectedRadioGroupItem: RadioGroupItem? by remember {
        mutableStateOf(RadioGroupItem.SHUTTER)
    }

    var selectorRadioGroupSelectedItem: IconRadioGroupItem? by remember {
        mutableStateOf(null)
    }
    var wbSelectorRadioGroupSelectedItem: IconRadioGroupItem? by remember {
        mutableStateOf(null)
    }
    var focusSelectorRadioGroupSelectedItem: IconRadioGroupItem? by remember {
        mutableStateOf(null)
    }
    ...
    val radioGroupItems: List<RadioButton> =
        remember(characteristics, magnifierValue) {

            listOf(
                TextRadioItem(
                    RadioGroupItem.SHUTTER,
                    characteristics.shutterValue,
                    "SHUTTER"
                ),
                TextRadioItem(RadioGroupItem.ISO, characteristics.isoValue, "ISO"),
                TextRadioItem(RadioGroupItem.WB, characteristics.wbValue, "WB"),
                TextRadioItem(
                    RadioGroupItem.FOCUS,
                    characteristics.focusValue,
                    "FOCUS"
                ),
                TextRadioItem(RadioGroupItem.MAGNIFIER, magnifierValue, "MAGNIFIER")
            )
    ...
    var autoItems by remember {
        mutableStateOf(
            mutableMapOf(
                RadioGroupItem.SHUTTER to false,
                RadioGroupItem.ISO to false,
                RadioGroupItem.WB to false,
                RadioGroupItem.FOCUS to false,
                RadioGroupItem.MAGNIFIER to false
            )
        )
    }
    ...
    LaunchedEffect(
        key1 = characteristics,
        key2 = radioGroupSelectedRadioGroupItem
    ) {

        isRadioGroupSelectorVisible = false
        isSelectorVisible = true
        selectorItems = when (radioGroupSelectedRadioGroupItem) {
            RadioGroupItem.SHUTTER -> {
            ...
            RadioGroupItem.WB -> {
                if (autoItems[RadioGroupItem.WB] == true) {
                    isRadioGroupSelectorVisible = true
            ...
            RadioGroupItem.FOCUS -> {
                if (autoItems[RadioGroupItem.FOCUS] == true) {
                    isRadioGroupSelectorVisible = true
                    isSelectorVisible = false
                }
        ...
        selectorRadioGroupItems = when (radioGroupSelectedRadioGroupItem) {
            RadioGroupItem.SHUTTER -> {
                null
            }
    ...
    LaunchedEffect(touchPoint) {
        if (touchPoint.isNotEmpty()) {
            settingsRequest = settingsRequest.copy(
                focusValue = if (autoItems[RadioGroupItem.FOCUS] == true) {
                    "A"
                } else {
                    characteristics.focusValue
                },
    ...
    LaunchedEffect(selectorSelectedItemIndex) {
        val selected = radioGroupSelectedRadioGroupItem
        if (selected != null) {
            autoItems = autoItems.toMutableMap().apply {
    ...
    Box(
        ...
        RadioGroup(
            ...
            onOptionSelected = { value ->
                radioGroupSelectedRadioGroupItem = value as RadioGroupItem?
            }
        )
        ...
        Column(
            ...
            Row(
                ...
                VectorShadow(
                    ...
                    vectorColor = if (autoItems[radioGroupSelectedRadioGroupItem] == true) Color.Green else Color.White,
                    shadowColor = Color.DarkGray,
                    resId = R.drawable.auto,
                    onClick = autoClick
                )
                ...
                Box(
                  ...
                  /*  val items by remember {
                    ...
                    if (isSelectorVisible) {
                        ValueSelector(
                            modifier = Modifier.height(42.dp),
                            position = when (radioGroupSelectedRadioGroupItem) {
                                RadioGroupItem.SHUTTER -> characteristics.shutterPosition
                                RadioGroupItem.ISO -> characteristics.isoPosition
                                RadioGroupItem.WB -> characteristics.wbPosition
                                RadioGroupItem.FOCUS -> characteristics.focusPosition
                                RadioGroupItem.MAGNIFIER -> magnifierPosition
                                null -> 0
                            },
                            items = selectorItems,
                            onSelectedItemChanged = { index, manual ->
